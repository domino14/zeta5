...
file `PrimeNumberTheoremAnd/Wiener.lean`. Licensed under the Apache License 2.0 (see `LICENSE` in this
...
import Zeta5.PNTPort.Fourier
import Zeta5.PNTPort.SmoothExistence
...
lemma limiting_fourier_lim1_aux (hcheby : cheby f) (hx : 0 < x) (C : ℝ) (hC : 0 ≤ C) :
  ...
  have l4 : nnabla a =O[atTop] (fun n : ℕ => (n ^ 2 * (Real.log n) ^ 2)⁻¹) := by
    convert (nnabla_bound C hx).comp_tendsto tendsto_natCast_atTop_atTop <;>
